...
const prisma = new PrismaClient(); //instanciando prismaclient
...
async function seedDatabase() {
  try {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      for (const service of services) {
        await prisma.barbershopService.create({
          data: {
            ...
            barbershop: {
              connect: {
                id: barbershop.id, //conecta o serviço à barbearia pelo id
              },
            },
      ...
      barbershops.push(barbershop); //adiciona cada barbershop criado no loop
    }
